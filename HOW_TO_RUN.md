...
- `alns` (Adaptive Large Neighborhood Search + SSGS decoder): `src/optimizer.py`
  - function: `alns_optimize(project, time_limit=...)`
...
python src/main.py sm_j10/PSP1.SCH --approach alns --workers 1 --time-budget 10
```
...
python src/main.py --batch sm_j10 --approach alns --workers 1 --time-budget 28
```
...
```powershell
python src/experiments.py --folders sm_j10 sm_j20 --approaches topo_seq id_ssgs lft_ssgs ga alns --time-budget 28 --workers 1
```
...
```powershell
python src/experiments.py --folders sm_j10 --approaches topological lft genetic alns --time-budget 5 --workers 1 --limit 10
```
...
- `alns` also supports portfolio parallelism, but for `sm_j10/sm_j20` one worker is usually fastest per unit improvement.
